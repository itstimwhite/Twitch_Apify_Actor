...
exports.unformatNumbers = (numStr) => {
  const numberMatch = numStr.replace(/[^0-9,.]/gi, "");
  if (numberMatch) {
    const number = parseFloat(numberMatch.replace(/,/g, ""));
    const multiplierMatch = numStr.match(/(?<=[0-9 ])[mkb]/gi);
...
module.exports.proxyConfiguration = async ({ proxyConfig, required = true, force = Apify.isAtHome(), blacklist = [""], hint = [] }) => {
